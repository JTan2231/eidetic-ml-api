...
class ColBERT(BaseColBERT):
    ...
    def __init__(self, name='bert-base-uncased', colbert_config=None):
        ...
        self.use_gpu = True#colbert_config.total_visible_gpus > 0

        ColBERT.try_load_torch_extensions(self.use_gpu)

        if self.colbert_config and self.colbert_config.mask_punctuation:
    ...
    def score_embeddings_against_query(self, Q, E, E_mask, only_indices=True):
        """
        Rank the entries E by their cosine similarity to the query Q

        Args:
            Q: PyTorch tensor representing the query embedding
            E: PyTorch tensor representing the entry embeddings
            E_mask: PyTorch tensor representing the entry masks

        Returns:
            List of indices corresponding to the given entries sorted according to their similarity to Q
        """


        # Repeat each query encoding for every corresponding document.
        Q_duplicated = Q.repeat_interleave(self.colbert_config.nway, dim=0).contiguous()
        scores = self.score(Q_duplicated, E, E_mask)

        n = len(scores)
        rankings = [(i, score.item()) for i, score in zip(range(n), scores)]
        rankings = sorted(rankings, key=lambda x: x[1], reverse=True)

        if only_indices:
            rankings = [r[0] for r in rankings]

        return rankings

    def compare_query_to_entries(self, Q, E, E_mask):
        Q = self.query(*Q)

        Q = Q.float()
        E = E.float()

        rankings = self.score_embeddings_against_query(Q, E, E_mask)

        return rankings

    def compare_entry_to_entries(self, Q, E, E_mask, ids):
        Q = Q.float()
        E = E.float()
